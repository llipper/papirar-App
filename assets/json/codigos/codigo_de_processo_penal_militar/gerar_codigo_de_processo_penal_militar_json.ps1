# gerar_codigo_de_processo_penal_militar_json.ps1
# Script dedicado exclusivamente ao Código de Processo Penal Militar (Decreto-Lei 1.002/1969).
# Lê o arquivo TXT (fonte da verdade) e gera um codigo_de_processo_penal_militar.json correto, completo e bem estruturado.
# Uso:  powershell -ExecutionPolicy Bypass -File gerar_codigo_de_processo_penal_militar_json.ps1
#       powershell -File gerar_codigo_de_processo_penal_militar_json.ps1 -MaxArticles 50   # para teste rápido

[CmdletBinding()]
param(
    [int]$MaxArticles = 0,   # 0 = processar todos (~718)
    [switch]$NoBackup
)
...
$txtPath = Join-Path $baseDir 'codigo_de_processo_penal_militar.txt'
$outJsonPath = Join-Path $baseDir 'codigo_de_processo_penal_militar.json'
...
while ($i -lt $totalLines) {
    ...
    if ($trim -like 'Brasília, 10 de janeiro de 2002*' -or $trim -like 'Rio de Janeiro, em 3 de outubro de 1941*' -or $trim -like 'Brasília, 21 de outubro de 1969*') {
    ...
    if ($trim -match $reArt) {
        ...
        while ($i -lt $totalLines) {
            ...
            if ($peek -like 'Rio de Janeiro, em 3 de outubro*' -or $peek -like 'Brasília, *' -or $peek -like 'Brasília, 21 de outubro de 1969*') { break }
        ...
        while ($i -lt $totalLines) {
            ...
            if ($l -like 'Rio de Janeiro, em 3 de outubro*' -or $l -like 'Brasília, *' -or $l -like 'Brasília, 21 de outubro de 1969*') { break }
    ...
    if ($inPreambulo) {
        ...
        if ($trim -match '^(DECRETO-LEI|Código de Processo Penal Militar|Os Ministros|CÓDIGO DE PROCESSO PENAL MILITAR|LIVRO I$)' -or $preambulo.Count -lt 6) {
            $preambulo += [ordered]@{
                texto = $trim
                linha = $ln
            }
...
$root = [ordered]@{
    ...
    fonteHtmlLocal   = 'codigos/Codigo_de_Processo_Penal_Militar/del1002.htm'
...
if ($finalArtCount -lt 600 -and $MaxArticles -eq 0) {
