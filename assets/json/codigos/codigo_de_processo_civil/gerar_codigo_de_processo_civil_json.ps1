# gerar_codigo_de_processo_civil_json.ps1
# Script dedicado exclusivamente ao Código de Processo Civil (Lei 13.105/2015).
# Lê o arquivo TXT (fonte da verdade) e gera um codigo_de_processo_civil.json correto, completo e bem estruturado.
# Uso:  powershell -ExecutionPolicy Bypass -File gerar_codigo_de_processo_civil_json.ps1
#       powershell -File gerar_codigo_de_processo_civil_json.ps1 -MaxArticles 50   # para teste rápido

[CmdletBinding()]
param(
    [int]$MaxArticles = 0,   # 0 = processar todos (~1072)
    [switch]$NoBackup
)
...
$txtPath = Join-Path $baseDir 'codigo_de_processo_civil.txt'
$outJsonPath = Join-Path $baseDir 'codigo_de_processo_civil.json'
...
while ($i -lt $totalLines) {
    ...
    if ($trim -match $reArt) {
        ...
        $caput = $trim -replace '^[\s]*Art\.[\s]*[\d.]+º?[\s.]*', ''
        $artStartLine = $ln
        $i++
    ...
    if ($inPreambulo) {
        ...
        if ($trim -match '^(LEI|Código de Processo Civil|A PRESIDENTA|P A R T E|PARTE GERAL|LIVRO I$)' -or $preambulo.Count -lt 6) {
            $preambulo += [ordered]@{
                texto = $trim
                linha = $ln
            }
...
$root = [ordered]@{
    ...
    sigla            = 'CPC'
    fonte            = 'Codigo_de_Processo_Civil.txt'
    fonteHtmlLocal   = 'codigos/Codigo_de_Processo_Civil/l13105.htm'
...
if ($finalArtCount -lt 1000 -and $MaxArticles -eq 0) {
