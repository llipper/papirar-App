# gerar_codigo_de_transito_brasileiro_json.ps1
# Script dedicado exclusivamente ao Código de Trânsito Brasileiro (Lei 9.503/1997).
# Lê o arquivo TXT (fonte da verdade) e gera um codigo_de_transito_brasileiro.json correto, completo e bem estruturado.
# Uso:  powershell -ExecutionPolicy Bypass -File gerar_codigo_de_transito_brasileiro_json.ps1
#       powershell -File gerar_codigo_de_transito_brasileiro_json.ps1 -MaxArticles 50   # para teste rápido

[CmdletBinding()]
param(
    [int]$MaxArticles = 0,   # 0 = processar todos (~341)
    [switch]$NoBackup
)
...
$txtPath = Join-Path $baseDir 'codigo_de_transito_brasileiro.txt'
$outJsonPath = Join-Path $baseDir 'codigo_de_transito_brasileiro.json'
...
while ($i -lt $totalLines) {
    ...
    if ($trim -like 'Brasília, 10 de janeiro de 2002*' -or $trim -like 'Rio de Janeiro, em 3 de outubro de 1941*' -or $trim -like 'Brasília, 21 de outubro de 1969*' -or $trim -like 'Brasília, 23 de setembro de 1997*') {
    ...
    if ($div) {
        ...
        # (e.g. after "CAPÍTULO I" comes the name). Preserves text from TXT.
        $titleLines = @()
        $j = $i + 1
    ...
    if ($trim -match $reArt) {
        ...
        while ($i -lt $totalLines) {
            ...
            if ($peek -like 'Rio de Janeiro, em 3 de outubro*' -or $peek -like 'Brasília, *' -or $peek -like 'Brasília, 21 de outubro de 1969*' -or $peek -like 'Brasília, 23 de setembro de 1997*') { break }
        ...
        while ($i -lt $totalLines) {
            ...
            if ($l -like 'Rio de Janeiro, em 3 de outubro*' -or $l -like 'Brasília, *' -or $l -like 'Brasília, 21 de outubro de 1969*' -or $l -like 'Brasília, 23 de setembro de 1997*') { break }
    ...
    if ($inPreambulo) {
        ...
        if ($trim -match '^(LEI Nº 9.503|Institui o Código de Trânsito Brasileiro|O PRESIDENTE DA REPÚBLICA|CÓDIGO DE TRÂNSITO|CAPÍTULO I$)' -or $preambulo.Count -lt 6) {
            $preambulo += [ordered]@{
                texto = $trim
                linha = $ln
            }
...
$root = [ordered]@{
    ...
    sigla            = 'CTB'
    fonte            = 'Codigo_de_Transito_Brasileiro.txt'
    fonteHtmlLocal   = 'codigos/Codigo_de_Transito_Brasileiro/l9503compilado.html'
    fonteOficial     = 'https://www.planalto.gov.br/ccivil_03/leis/l9503.htm'
