# gerar_codigo_penal_json.ps1
# Script dedicado exclusivamente ao Código Penal (Decreto-Lei 2.848/1940).
# Lê o arquivo TXT (fonte da verdade) e gera um codigo_penal.json correto, completo e bem estruturado.
# A única diferença para os outros códigos é preservar os objetos "audio" (explicações em MP3) que estavam no JSON original.
# Uso:  powershell -ExecutionPolicy Bypass -File gerar_codigo_penal_json.ps1
#       powershell -File gerar_codigo_penal_json.ps1 -MaxArticles 50   # para teste rápido

[CmdletBinding()]
param(
    [int]$MaxArticles = 0,   # 0 = processar todos (~361)
    [switch]$NoBackup
)
...
$txtPath = Join-Path $baseDir 'codigo_penal.txt'
$outJsonPath = Join-Path $baseDir 'codigo_penal.json'
...
while ($i -lt $totalLines) {
    ...
    # Stricter to avoid catching division title names (DA/DO/DOS...) that now go to div.titulo.
    ...
    if ($div) {
        ...
        # (e.g. after "TÍTULO I" comes "DA APLICAÇÃO DA LEI PENAL MILITAR").
        ...
        while ($j -lt $totalLines) {
            ...
            # Looks like a division descriptive name: must be ALL-UPPER (no lowercase a-z).
            # Use -cmatch for case-sensitive so mixed-case rubricas are not included.
    ...
    if ($trim -match $reArt) {
        ...
        while ($i -lt $totalLines) {
            ...
            if ($peek -match '^(Pena |Pena –|Pena -)') { break }
        ...
        while ($i -lt $totalLines) {
            ...
            if ($l -match '^(Pena |Pena –|Pena -)') { break }

            # "Pena - ..." handling for Código Penal (keeps the pattern clean, "Pena" as separate field)
            if ($l -match '^(Pena |Pena –|Pena -)\s*(.*)$') {
                $art | Add-Member -NotePropertyName pena -NotePropertyValue $Matches[2] -Force
                $i++
                continue
            }
    ...
    if ($inPreambulo) {
        ...
        if ($trim -match '^(DECRETO-LEI No 2.848|Código Penal|O PRESIDENTE DA REPÚBLICA|PARTE GERAL|TÍTULO I$)' -or $preambulo.Count -lt 6) {
            $preambulo += [ordered]@{
                texto = $trim
                linha = $ln
            }
...
$root = [ordered]@{
    ...
    sigla            = 'CP'
    fonte            = 'Codigo_Penal.txt'
    fonteHtmlLocal   = 'codigos/Codigo_Penal/del2848.htm'
...
try {
  ...
  if ($gitRoot) {
    $oldPath = "assets/json/codigos/codigo_penal/codigo_penal.json"
          ...
          foreach ($n in $nodes) {
            if ($n.artigos) {
              foreach ($a in $n.artigos) {
                ...
                if ($a.incisos) { CollectAudios $a.incisos $map }
                if ($a.paragrafos) { CollectAudios $a.paragrafos $map }
                if ($a.alineas) { CollectAudios $a.alineas $map }
              }
            }
            if ($n.divisoes) { CollectAudios $n.divisoes $map }
          }
        }
        $audioMap = @{}
          ...
          foreach ($n in $nodes) {
            if ($n.artigos) {
              foreach ($a in $n.artigos) {
                ...
                if ($a.incisos) { AssignAudios $a.incisos }
                if ($a.paragrafos) { AssignAudios $a.paragrafos }
                if ($a.alineas) { AssignAudios $a.alineas }
              }
            }
            if ($n.divisoes) { AssignAudios $n.divisoes }
          }
        }
        AssignAudios $root.divisoes
        Write-Host "Merged audio explanations from git version (preserved as per requirement)."
      }
    }
  }
} catch {
