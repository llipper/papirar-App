# gerar_codigo_civil_json.ps1
# Script dedicado exclusivamente ao Código Civil (Lei 10.406/2002).
# Lê o arquivo TXT (fonte da verdade) e gera um codigo_civil.json correto, completo e bem estruturado.
# Uso:  powershell -ExecutionPolicy Bypass -File gerar_codigo_civil_json.ps1
#       powershell -File gerar_codigo_civil_json.ps1 -MaxArticles 50   # para teste rápido

[CmdletBinding()]
param(
    [int]$MaxArticles = 0,   # 0 = processar todos (2046)
    [switch]$NoBackup
)
...
$txtPath = Join-Path $baseDir 'codigo_civil.txt'
$outJsonPath = Join-Path $baseDir 'codigo_civil.json'
...
function Clean-Text([string]$t) {
    ...
    $t = $t.Trim()
    # Remove o "o " / "º " residual do <sup>o</sup> do HTML (ex: "o Toda pessoa")
...
while ($i -lt $totalLines) {
    ...
    if ($inPreambulo) {
        # Só guarda as linhas "oficiais" do preâmbulo (evita lixo inicial do HTML convertido)
        if ($trim -match '^(LEI|Institui|O PRESIDENTE|P A R T E|LIVRO I$)' -or $preambulo.Count -lt 6) {
            $preambulo += [ordered]@{
                texto = $trim
                linha = $ln
            }
...
$root = [ordered]@{
    ...
    sigla            = 'CC'
    fonte            = 'Codigo_Civil.txt'
    fonteHtmlLocal   = 'codigos/Codigo_Civil/l10406compilada.htm'
...
Write-Host "Artigos no JSON gerado: $finalArtCount (esperado: 2046)" -ForegroundColor $(if ($finalArtCount -eq 2046) { 'Green' } else { 'Yellow' })
Write-Host "Divisões por tipo:"
$divStats.GetEnumerator() | Sort-Object Name | ForEach-Object { "  $($_.Key): $($_.Value)" }

if ($finalArtCount -ne 2046 -and $MaxArticles -eq 0) {
    Write-Warning "Contagem de artigos diferente de 2046. Verifique o parser ou o arquivo TXT."
}
