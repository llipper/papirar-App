# gerar_estatuto_do_idoso_json.ps1
# Gera estatuto_do_idoso.json a partir do TXT e preserva
# audios no formato separado: audioId no JSON principal + *_audio.json.
# Uso: powershell -ExecutionPolicy Bypass -File gerar_estatuto_do_idoso_json.ps1
...
$txtPath = Join-Path $baseDir 'estatuto_do_idoso.txt'
$outJsonPath = Join-Path $baseDir 'estatuto_do_idoso.json'
$audioJsonPath = Join-Path $baseDir 'estatuto_do_idoso_audio.json'
...
Write-Host "Lendo TXT do Estatuto do Idoso..." -ForegroundColor Cyan
...
function Get-AudioId([string]$numero) {
    ...
    $candidate = "estatuto_idoso-artigo_$slug"
...
function Add-MissingArt72IfNeeded([int]$linha) {
    if ($script:lastArticleNumber -ne '71') { return }

    $artigo = New-Artigo '72' 'Art. 72.' $linha '(VETADO)'
    Add-Article $artigo
    $script:currentArticle = $artigo
    $script:currentPart = $artigo
    $script:currentParagraph = $null
    $script:lastArticleNumber = '72'
    $script:artCount++
}
...
for ($i = 0; $i -lt $totalLines; $i++) {
    ...
    if ($trim -like 'Brasília, 1o de outubro de 2003*') {
        $script:inFecho = $true
        $script:currentArticle = $null
        $script:currentPart = $null
        $script:currentParagraph = $null
...
$root = [ordered]@{
    id             = 'Estatuto_do_Idoso'
    titulo         = 'Estatuto do Idoso'
    apelido        = 'Estatuto do Idoso.'
    sigla          = 'EI'
    fonte          = 'estatuto_do_idoso.txt'
    fonteHtmlLocal = 'l10.741.htm'
    fonteOficial   = 'https://www.planalto.gov.br/ccivil_03/leis/2003/l10.741.htm'
...
if ($finalArtCount -lt 118 -and $MaxArticles -eq 0) {
