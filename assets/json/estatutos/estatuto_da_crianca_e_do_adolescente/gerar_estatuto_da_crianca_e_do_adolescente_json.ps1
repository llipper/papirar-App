# gerar_estatuto_da_crianca_e_do_adolescente_json.ps1
# Gera estatuto_da_crianca_e_do_adolescente.json a partir do TXT e preserva
# audios no formato separado: audioId no JSON principal + *_audio.json.
# Uso: powershell -ExecutionPolicy Bypass -File gerar_estatuto_da_crianca_e_do_adolescente_json.ps1
...
$txtPath = Join-Path $baseDir 'estatuto_da_crianca_e_do_adolescente.txt'
$outJsonPath = Join-Path $baseDir 'estatuto_da_crianca_e_do_adolescente.json'
$audioJsonPath = Join-Path $baseDir 'estatuto_da_crianca_e_do_adolescente_audio.json'
...
Write-Host "Lendo TXT do ECA..." -ForegroundColor Cyan
...
for ($i = 0; $i -lt $totalLines; $i++) {
    ...
    if ($trim -like 'Brasília, 13 de julho de 1990*') {
...
$root = [ordered]@{
    id             = 'Estatuto_da_Crianca_e_do_Adolescente'
    titulo         = 'Estatuto da Criança e do Adolescente'
    apelido        = 'Estatuto da Criança e do Adolescente.'
    sigla          = 'ECA'
    fonte          = 'estatuto_da_crianca_e_do_adolescente.txt'
    fonteHtmlLocal = 'l8069.htm'
    fonteOficial   = 'https://www.planalto.gov.br/ccivil_03/leis/l8069.htm'
