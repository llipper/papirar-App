# gerar_estatuto_dos_militares_json.ps1
# Gera estatuto_dos_militares.json a partir do TXT e preserva
# audios no formato separado: audioId no JSON principal + *_audio.json.
# Uso: powershell -ExecutionPolicy Bypass -File gerar_estatuto_dos_militares_json.ps1
...
$txtPath = Join-Path $baseDir 'estatuto_dos_militares.txt'
$outJsonPath = Join-Path $baseDir 'estatuto_dos_militares.json'
$audioJsonPath = Join-Path $baseDir 'estatuto_dos_militares_audio.json'
...
Write-Host "Lendo TXT do Estatuto dos Militares..." -ForegroundColor Cyan
...
function Get-AudioId([string]$numero) {
    ...
    $candidate = "estatuto_militares-artigo_$slug"
...
$reArt = '^\s*Art\s*\.\s*(\d+(?:\.\d+)*)([ºo°]?)(?:-([A-Z]))?\s*\.?\s*(.*)$'
$reTitulo = '^\s*T[ÍI]TULO\s+[IVXLC]+'
$reCapitulo = '^\s*CAP[ÍI]TULO\s+[IVXLC]+'
$reSecao = '^\s*SE[ÇC][ÃA]O\s+[IVXLCÚ]+'
$reSubsecao = '^\s*SUBSE[ÇC][ÃA]O\s+[IVXLCÚ]+'
$reAnexo = '^\s*ANEXO\b'
...
for ($i = 0; $i -lt $totalLines; $i++) {
    ...
    if ($trim -like 'Brasília, 9 de dezembro de 1980*') {
        $script:inFecho = $true
        $script:currentArticle = $null
        $script:currentPart = $null
        $script:currentParagraph = $null
    ...
    if ($script:inFecho) {
        ...
        $script:fecho += (New-TextoItem $trim $ln)
        continue
    }
...
$root = [ordered]@{
    id             = 'Estatuto_dos_Militares'
    titulo         = 'Estatuto dos Militares'
    apelido        = 'Estatuto dos Militares.'
    sigla          = 'EM'
    fonte          = 'estatuto_dos_militares.txt'
    fonteHtmlLocal = 'l6880.htm'
    fonteOficial   = 'https://www.planalto.gov.br/ccivil_03/leis/l6880.htm'
