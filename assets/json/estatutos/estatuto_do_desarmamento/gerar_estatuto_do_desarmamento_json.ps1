# gerar_estatuto_do_desarmamento_json.ps1
# Gera estatuto_do_desarmamento.json a partir do TXT e preserva
# audios no formato separado: audioId no JSON principal + *_audio.json.
# Uso: powershell -ExecutionPolicy Bypass -File gerar_estatuto_do_desarmamento_json.ps1
...
$txtPath = Join-Path $baseDir 'estatuto_do_desarmamento.txt'
$outJsonPath = Join-Path $baseDir 'estatuto_do_desarmamento.json'
$audioJsonPath = Join-Path $baseDir 'estatuto_do_desarmamento_audio.json'
...
Write-Host "Lendo TXT do Estatuto do Desarmamento..." -ForegroundColor Cyan
...
function Normalize-Numero([string]$numero, [string]$ordinal, [string]$suffix) {
    $n = $numero.Trim()
...
function Get-AudioId([string]$numero) {
    ...
    $candidate = "estatuto_desarmamento-artigo_$slug"
...
function New-TextoItem([string]$texto, [int]$linha) {
    return [pscustomobject]@{
        tipo  = 'texto'
        texto = Clean-Text $texto
        linha  = $linha
...
function New-TaxaRow([string]$ato, [string]$valor) {
    return [pscustomobject]@{
        ato   = $ato
        valor = $valor
    }
...
function Add-DesarmamentoTaxTable($anexo) {
    if ($anexo.PSObject.Properties['tabela']) { return }

    $anexo | Add-Member -NotePropertyName tabela -NotePropertyValue ([pscustomobject]@{
        cabecalho = @('ATO ADMINISTRATIVO', 'R$')
        linhas    = @(
            New-TaxaRow 'I - Registro de arma de fogo: até 31 de dezembro de 2008 (art. 30)' 'Gratuito'
            New-TaxaRow 'I - Registro de arma de fogo: a partir de 1o de janeiro de 2009' '60,00'
            New-TaxaRow 'II - Renovação do certificado de registro de arma de fogo: até 31 de dezembro de 2008 (art. 5o, § 3o)' 'Gratuito'
            New-TaxaRow 'II - Renovação do certificado de registro de arma de fogo: a partir de 1o de janeiro de 2009' '60,00'
            New-TaxaRow 'III - Registro de arma de fogo para empresa de segurança privada e de transporte de valores' '60,00'
            New-TaxaRow 'IV - Renovação do certificado de registro de arma de fogo para empresa de segurança privada e de transporte de valores: até 30 de junho de 2008' '30,00'
            New-TaxaRow 'IV - Renovação do certificado de registro de arma de fogo para empresa de segurança privada e de transporte de valores: de 1o de julho de 2008 a 31 de outubro de 2008' '45,00'
            New-TaxaRow 'IV - Renovação do certificado de registro de arma de fogo para empresa de segurança privada e de transporte de valores: a partir de 1o de novembro de 2008' '60,00'
            New-TaxaRow 'V - Expedição de porte de arma de fogo' '1.000,00'
            New-TaxaRow 'VI - Renovação de porte de arma de fogo' '1.000,00'
            New-TaxaRow 'VII - Expedição de segunda via de certificado de registro de arma de fogo' '60,00'
            New-TaxaRow 'VIII - Expedição de segunda via de porte de arma de fogo' '60,00'
        )
    })
}
...
$script:inText = $false
$script:inFecho = $false
$script:artCount = 0
$script:lastArticleNumber = $null

$reArt = '^\s*Art\.?\s*(\d+(?:\.\d+)*)([ºo°]?)(?:-([A-Z]))?\s*\.?\s*(.*)$'
$reCapitulo = '^\s*CAP[ÍI]TULO\s+[IVXLC]+'
$reSecao = '^\s*Se[çc][ãa]o\s+[IVXLCÚú]+'
$reSubsecao = '^\s*Subse[çc][ãa]o\s+[IVXLCÚú]+'
$reAnexo = '^\s*ANEXO\b'
$reInciso = '^\s*([IVXLCDM]+)\s*[-–]\s*(.*)$'
$reParagrafo = '^\s*(Parágrafo único|§\s*(\d+)\s*[ºo°]?(?:-([A-Z]))?)\.?\s*-?\s*(.*)$'
$reAlinea = '^\s*([a-z])\)\s*(.*)$'
...
function Is-StructuralLine([string]$line) {
    return (
        ...
        $line -match $reAnexo -or
        $line -match $reArt
    )
}

function Add-MissingArt36IfNeeded([int]$linha) {
    if ($script:lastArticleNumber -ne '35') { return }

    $artigo = New-Artigo '36' 'Art. 36.' $linha 'É revogada a Lei no 9.437, de 20 de fevereiro de 1997.'
    Add-Article $artigo
    $script:currentArticle = $artigo
    $script:currentPart = $artigo
    $script:currentParagraph = $null
    $script:lastArticleNumber = '36'
    $script:artCount++
}
...
for ($i = 0; $i -lt $totalLines; $i++) {
    ...
    if ($trim -like 'Brasília, 22 de dezembro de 2003*') {
        $script:inFecho = $true
        $script:currentArticle = $null
        $script:currentPart = $null
        $script:currentParagraph = $null
    ...
    if ($script:inFecho) {
        if ($trim -match $reAnexo) {
            $division = New-Division 'anexo' $trim $ln
            Add-Division $division
            $script:inFecho = $false
            continue
        }

        $script:fecho += [pscustomobject]@{ tipo = 'texto'; texto = Clean-Text $trim; linha = $ln }
        continue
    }

    if ($trim -match $reCapitulo) {
        $script:inText = $true
    }
    ...
    if ($script:pendingTitleFor -and -not (Is-StructuralLine $trim)) {
        ...
        if ($script:pendingTitleFor.tipo -eq 'anexo' -and (Clean-Text $trim) -eq 'TABELA DE TAXAS') {
            Add-DesarmamentoTaxTable $script:pendingTitleFor
        }
        $script:pendingTitleFor = $null
        continue
    }

    if ($script:currentContainer -and $script:currentContainer.tipo -eq 'anexo' -and -not $script:currentArticle) {
        if (-not $script:currentContainer.PSObject.Properties['itens']) {
            $script:currentContainer | Add-Member -NotePropertyName itens -NotePropertyValue ([System.Collections.ArrayList]::new())
        }
        [void]$script:currentContainer.itens.Add((New-TextoItem $trim $ln))
        continue
    }

    if ($trim -match $reArt) {
        $numero = Normalize-Numero $Matches[1] $Matches[2] $Matches[3]
        $caput = $Matches[4]
        if ($numero -eq '37') {
            Add-MissingArt36IfNeeded ($ln - 1)
        }

        $rotulo = if ($numero -match 'º$') { "Art. $numero" } else { "Art. $numero." }
        $script:currentArticle = New-Artigo $numero $rotulo $ln $caput
        Add-Article $script:currentArticle
        $script:currentPart = $script:currentArticle
        $script:currentParagraph = $null
        $script:lastArticleNumber = $numero
        $script:artCount++
    ...
    if ($trim -match $reParagrafo) {
        ...
        $paragraphSuffix = $Matches[3]
        $paragraphText = $Matches[4]
        $numeroParagrafo = if ($rawLabel -match 'Parágrafo único') {
            'único'
        } else {
            Normalize-Numero $paragraphNumber 'º' $paragraphSuffix
        }
        $paragrafo = New-Paragrafo $numeroParagrafo $rawLabel $ln $paragraphText
...
if ($script:fecho.Count -gt 0) {
    $script:divisoes += [pscustomobject]@{
        tipo  = 'fecho'
        linha  = $script:fecho[0].linha
        itens = $script:fecho
    }
}

$root = [ordered]@{
    id             = 'Estatuto_do_Desarmamento'
    titulo         = 'Estatuto do Desarmamento'
    apelido        = 'Estatuto do Desarmamento.'
    sigla          = 'ED'
    fonte          = 'estatuto_do_desarmamento.txt'
    fonteHtmlLocal = 'l10.826.htm'
    fonteOficial   = 'https://www.planalto.gov.br/ccivil_03/leis/2003/l10.826.htm'
...
if ($finalArtCount -lt 40 -and $MaxArticles -eq 0) {
